...
public:
	...
	Layer(in_width*in_height*in_height*in_channel,(in_width/pooling_width)*(in_height/pooling_height)*in_channel),
	...
	void forward(const Matrix& prev_layer_data)
	{
		...
		for (int channel_start = 0; channel_start < channel_end; channel_start += channel_stride)
		{
			...
			for (int col_start = channel_start; col_start < col_end; col_start += channel_stride)
			{
